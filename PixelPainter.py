...
class PixelPainter:
    ...
    '''
    pixel_w = 20  # initial on screen pixel dimension
    n_row_default = 28 ## default
    n_col_default = 48 ## default
    ...
    def printbitmap(self):
        ...
        #print(str(g_bitmap))
        #print(str(b_bitmap))
        bytes =r_bitmap.getBytes()

        for index in range(len(bytes)):
            bytes[index] = int('{:08b}'.format(bytes[index])[::-1], 2)

        width = 6
        for col in range(0,6):
            print( "LOGO%d     dc     "% col,end="")
            for row in range(27,-1,-1):
                index = col+(row*width)
                print ("$%02X" % bytes[index], end = "")
                if row == 14:
                    print("\n          dc     ",end ="")
                elif row ==0:
                    print()
